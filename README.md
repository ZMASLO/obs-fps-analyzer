...
- **FPS trace** ("Record FPS trace (per-frame CSV)"): while enabled, writes `fpstrace_<label>_<timestamp>.csv` into the dump folder — one row per analysed frame (analysis timestamp, diff %, unique/tearing decision, frametime, EMA, per-frame FPS) and one per published tick (window, FPS, frametime). A few KB per second. These traces are the fixtures the offline test tools replay to pin the FPS algorithm's behaviour across versions.
